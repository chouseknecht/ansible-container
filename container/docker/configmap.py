...
class DockerConfigmapMixin(object):
    """
    Methods needed to simulate K8s ConfigMap objects
    """

    @property
    def configmap_volume_name(self):
        return "{}_config".format(self.project_name)

    def get_configmap_volume(self):
        result = None
        try:
            for vol in self.client.volumes.list(filters={'name': self.configmap_volume_name}):
                if vol.name == self.configmap_volume_name:
                    result = vol
                    break
        except docker_errors.APIError as exc:
            raise exceptions.AnsibleContainerException(
                "Error fetching volume {}: {}".format(self.configmap_volume_name, str(exc))
            )
        return result

    def create_configmap_volume(self):
        ...
        if not volume_obj:
            ...
            try:
                volume_obj = self.client.volumes.create(name=self.configmap_volume_name, labels=labels)
            except docker_errors.APIError as exc:
                raise exceptions.AnsibleContainerException(
                    "Error creating configmap volume: {}".format(str(exc))
                )
